...
public class DatabaseConnector {
	...
	public School getSchoolToEdit(String schoolId) {
		String hql = "FROM School WHERE id=" + schoolId;
		Query query = session.createQuery(hql);
		List<School> schools = query.list();
		School schoolToEdit=schools.get(0);
		return schoolToEdit;
	}
	...
	public void updateSchool(School schoolToEdit) {
		Transaction transaction = session.beginTransaction();
		session.update(schoolToEdit);
		transaction.commit();
	}
	...
	public SchoolClass getSchoolClassToEdit(String schoolClassId) {
		String hql = "FROM SchoolClass WHERE id=" + schoolClassId;
		Query query = session.createQuery(hql);
		List<SchoolClass> schoolClasses = query.list();
		SchoolClass schoolClassToEdit=schoolClasses.get(0);
		return schoolClassToEdit;
	}
	
	public School getSchoolOfEditedClass(String schoolClassId) {
		String hql = "FROM SchoolClass WHERE id=" + schoolClassId;
		Query query = session.createQuery(hql);
		List<SchoolClass> schoolClasses = query.list();
		SchoolClass schoolClassToEdit=schoolClasses.get(0);
		int schoolId = schoolClassToEdit.getSchool_id();
		hql = "FROM School WHERE id=" + schoolId;
		query = session.createQuery(hql);
		List<School> schools = query.list();
		return schools.get(0);
	}
	
	public void updateSchoolClass(SchoolClass schoolClassToEdit) {
		Transaction transaction = session.beginTransaction();
		session.update(schoolClassToEdit);
		transaction.commit();
	}
